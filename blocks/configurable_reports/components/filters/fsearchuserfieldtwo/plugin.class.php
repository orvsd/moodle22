...
class plugin_fsearchuserfieldtwo extends plugin_base{
	
	function init(){
		$this->form = true;
		$this->unique = true;
		$this->fullname = get_string('fsearchuserfieldtwo','block_configurable_reports');
	...
	private function execute_sql($finalelements, $data) {
		...
		if($filter_fuserfield &&
		   preg_match("/%%FILTER_USERS2:([^%]+)%%/i",$finalelements, $output)){
			$replace = ' AND '.$output[1].' LIKE '. "'%$filter_fuserfield%'";
			return str_replace('%%FILTER_USERS2:'.$output[1].'%%',$replace,$finalelements);
		}
