...
class theme_krystle2_core_renderer extends core_renderer {
...
class theme_krystle2_topsettings_renderer extends plugin_renderer_base {
    ...
    public function navigation_tree(global_navigation $navigation) {
        ...
        $content .= html_writer::empty_tag('img', array('alt' => '', 'src' =>$this->pix_url('menu/home_icon', 'theme')));
        $content .= html_writer::end_tag('a');
        $content .= html_writer::end_tag('li');
    if(isloggedin()) {
        $content .=html_writer::start_tag('li');
        $content .=html_writer::start_tag('a', array('href' => new moodle_url('/login/logout.php'), 'id'=>'key'));
        $content .= html_writer::empty_tag('img', array('alt' => '', 'src' =>$this->pix_url('menu/logging', 'theme')));
        $content .= html_writer::end_tag('a');
        $content .= html_writer::end_tag('li');
    } else {
        $content .=html_writer::start_tag('li');
        $content .=html_writer::start_tag('a', array('href' => new moodle_url('/login/index.php'), 'id'=>'key'));
        $content .= html_writer::empty_tag('img', array('alt' => '', 'src' =>$this->pix_url('menu/logging', 'theme')));
        $content .= html_writer::end_tag('a');
        $content .= html_writer::end_tag('li');
    }
    ...
    protected function navigation_node(navigation_node $node, $attrs=array()) {
        ...
        $lis = array();
        $dummypage = new krystle2_dummy_page();
        ...
        foreach ($items as $item) {
            ...
            if(substr($item->id, 0, 17)=='expandable_branch' && $item->children->count()==0) {
                ...
                $subnav = new krystle2_expand_navigation($dummypage, $item->type, $item->key);
