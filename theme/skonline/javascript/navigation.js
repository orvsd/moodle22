/**
 * Customises the dock for the skonline theme and does some other cool stuff
 */
function customise_dock_for_theme() {
    ...
    M.core_dock.on('dock:drawcompleted', skonline.dock.enable_side_switching, skonline.dock);
    // When the dock is first drawn check to see if it should be moved
    M.core_dock.on('dock:drawstarted', skonline.dock.check_initial_position, skonline.dock);
    // Corrects the panel x position for the theme
    M.core_dock.on('dock:itemadded', function(item) {
        item.on('dockeditem:showstart', skonline.dock.correct_panel_x_position, skonline.dock, item);
        item.on('dockeditem:resizecomplete', skonline.dock.correct_panel_x_position, skonline.dock, item);
    });

    // Override the default fix_title_orientation method with our skonline method
    // this will use SVG and rotate the text if possible.
    M.core_dock.genericblock.prototype.fix_title_orientation = skonline.dock.fix_title_orientation;
    M.core_dock.genericblock.prototype.resize_block_space = skonline.dock.resize_block_space;
    return true;
}

var skonline = (function(){
    return {
        namespaces : {
        ...
        dock : {
            ...
            fix_title_orientation : function (node) {
                if (M.core_dock.cfg.orientation == 'vertical') {
                    return skonline.transform.make_vertical_text(node);
                }
                return node;
            },
        ...
        transform : {
            make_vertical_text : function(node) {
                ...
                var txt = document.createElementNS(skonline.namespaces.svg, 'text');
                txt.setAttribute('x', '0');
                txt.setAttribute('y', '0');
                txt.setAttribute('transform','rotate(90, 5, 5)');
                txt.appendChild(document.createTextNode(node.firstChild.nodeValue));

                var svg = document.createElementNS(skonline.namespaces.svg, 'svg');
